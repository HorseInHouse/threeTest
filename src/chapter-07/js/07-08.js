  ...
  function createPointCloud(name, texture, size, transparent, opacity, sizeAttenuation, color) {
    ...
    for (var i = 0; i < 150; i++) {
      var particle = new THREE.Vector3(
        ...
        Math.random() * range - range / 2);
      particle.ljzY = 0.1 + Math.random() / 5;
      particle.ljzX = (Math.random() - 0.5) / 3;
      particle.ljzZ = (Math.random() - 0.5) / 3;
    ...
    scene.children.forEach(function (child) {
      if (child instanceof THREE.Points) {
        ...
        vertices.forEach(function (v) {
          ...
          if (v.y <= 0) v.y = 60;
          if (v.x <= -20 || v.x >= 20) v.ljzX = v.ljzX * -1;
          if (v.z <= -20 || v.z >= 20) v.ljzZ = v.ljzZ * -1;
        });
